import { BadRequestException, Controller, HttpException, HttpStatus, NotFoundException, Post } from '@nestjs/common';
...
export class AuthController {
    ...
    async login(@Req() req:Request): Promise<string> {
        ...
        let checkLogin = await this.authService.login(email, password);
        if(checkLogin.check) {
    ...
    async signUp(@Req() req:Request): Promise<string> {
        ...
        let signUpCheck = await this.authService.signUp(email, password, name, phone,birthday,gender);
