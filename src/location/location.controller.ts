import { Body, Controller, Get, Post, Req, UseGuards,Param, Put, Delete } from '@nestjs/common';
...
export class LocationController {
    ...
    getLocationPaginate(@Req() req: Request): Promise<location[]> {
        ...
        return this.locationService.locationPagination(parseInt(pageIndex), parseInt(pageSize))
    }

    @ApiParam({ name: "location_id" })
    @Get(":location_id")
    getLocationByID(@Param("location_id") location_id: string): Promise<location> {

        return this.locationService.getLocationByID(location_id)
    }

    @Put(":location_id")
    updateLocationByID(@Param("location_id") location_id: string,@Body() location: LocationUpdateDto): Promise<location>{
    ...
    @ApiParam({ name: "location_id" })
    @Delete(":location_id")
    deleteLocationByID(@Param("location_id") location_id: string): Promise<LocationResultDto> {
